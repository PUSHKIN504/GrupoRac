...
@Component({
    selector: 'app-login',
    templateUrl: './reportvehiculo.component.html',
})
export class ReporteVehiculoComponent implements OnInit{
   ...
   async ngOnInit() {
      ...
      await this.reporteService.reporteVehiculos()
        .then(data => {
          this.vehiculos = data;
        ...
        doc.setFontSize(15);
        doc.setFont(undefined, 'bold');
        doc.text('Reporte de Vehiculos', margin + 10, margin + imgHeight + 70);

        autoTable(doc, {
            head: [['Placa', 'Modelo', 'Marca', 'Sede', 'Factuar de Compra', 'Precio']],
            body: cuerpo,
