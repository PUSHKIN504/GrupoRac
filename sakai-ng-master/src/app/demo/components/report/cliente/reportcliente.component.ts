...
export class ReporteClienteComponent implements OnInit{
    ...
    ciudadDDL: SelectItem[] = [];
    selectedCiudad?: string = '0601';
    ...
    constructor(private router: Router, private reporteService: ReporteService, private ciudadService: ServiceService,private renderer: Renderer2, 
   ...
   async ngOnInit() {
      ...
      await this.ciudadService.getMunicipios().subscribe((data: any)=>{
        this.municipios = data;

        this.municipios.forEach(element => {
          this.ciudadDDL.push({label: element.ciu_Descripcion, value: element.ciu_Id})
        });
        console.log(this.ciudadDDL)
      });


      await this.reporteService.reporteClientes(this.selectedCiudad)
        .then(data => {
          this.clientes = data;
          console.log(data, "data")


          const cuerpo = this.clientes.map(item => [
            item.cli_Id?.toString(),
            item.cli_Nombre?.toString(),
            item.cli_Apellido?.toString(),
            item.cli_FechaNac?.toString(),
            item.cli_Sexo?.toString(),
            item.cli_DNI?.toString(),
            item.ciu_Id?.toString(),
            item.cli_Direccion?.toString(),
          ]);
          console.log(cuerpo)
        ...
        doc.setFontSize(15);
        doc.setFont(undefined, 'bold');
        doc.text('Reporte de Clientes', margin + 10, margin + imgHeight + 70);

        autoTable(doc, {
            head: [['Id Cliente', 'Nombre', 'Apellido', 'Fecha de Nacimiento', 'Sexo', 'DNI', 'Ciudad', 'Direccion']],
            body: cuerpo,
