...
@Component({
    selector: 'app-login',
    templateUrl: './reportcompra.component.html',
})
export class ReporteCompraComponent implements OnInit{
    ...
    constructor(private router: Router, private service: ReporteService, private renderer: Renderer2, 
   ...
   async ngOnInit() {
      ...
      await this.service.reporteCompras(this.selectedMonth,this.selectedYear)
        .then(data => {
          this.compras = data;
          console.log(data, "data")


          const cuerpo = this.compras.map(item => [
        ...
        doc.setFontSize(15);
        doc.setFont(undefined, 'bold');
        doc.text('Reporte de Compras', margin + 10, margin + imgHeight + 70);
