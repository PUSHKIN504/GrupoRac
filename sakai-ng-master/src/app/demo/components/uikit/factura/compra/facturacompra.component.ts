...
export class FacturaCompraComponent implements OnInit {
    ...
    async ngOnInit() {
        ...
        await this.CompraService.List().then((data) => {
            this.compras = data;
            this.loading = false;
            console.log(data);
        });
        ...
        await this.modeloService.getModelo().subscribe(
            (data: any) => {
                this.modelos = data;
    ...
    async editFactura(compra: Compra) {
        ...
        await this.VehiculoService.FindDetalle(this.compra.com_Id).then(
            (data) => {
                this.vehiculos = data;
    ...
    saveModalVehiculo() {
        ...
        if (this.vehiculo.veh_Placa?.trim()) {
            if (this.vehiculo.veh_Id) {
                ...
                this.vehiculo.mod_Id = this.findModelByDescripcion(
                    this.vehiculo.mod_Descripcion
                );
                this.vehiculos[this.findIndexById(this.vehiculo.veh_Id)] =
                    this.vehiculo;
            ...
            //visiblidad
            // const vehiculoShow = document.getElementById(
            //     `vehiculoShow-${this.vehiculo.veh_Placa}`
            // );
            // const vehiculoInput = document.getElementById(
            //     `vehiculoInput-${this.vehiculo.veh_Placa}`
            // );
    ...
    async emitirFactura() {
        ...
        if (this.vehiculos.length > 0) {
            await this.CompraService.Emitir(this.compra).then((result) => {
                if (result.code == 200) {
                    this.messageService.add({
                        severity: 'success',
                        summary: 'Éxito',
                        detail: 'Emición ejecutada correctamente',
                        life: 3000,
                    });
                    ...
                    this.messageService.add({
                        severity: 'warning',
                        summary: 'Advertencia',
                        detail: `Algo salió mal | Código: ${result.code}`,
                        life: 3000,
                    });
                }
            }),
                (error) => {
                    this.messageService.add({
                        severity: 'error',
                        summary: 'Error',
                        detail: `Algo salió mal | Error: ${error}`,
                        life: 3000,
                    });
                };
        ...
        await this.clienteService.getClientes().subscribe(
            (data: any) => {
                ...
                this.clientes = data;
                this.cliente =
                    this.clientes[this.findClienteByDNI(this.compra.cli_DNI)];
            },
            (error) => {
                console.log(error);
            }
        ...
        if (this.vehiculos.length > 0) {
            ...
            doc.text('Grupo Rac, Sur', imgWidth - margin - 10, margin + 20, {
                align: 'right',
            });
            ...
            doc.text(
                'Dirección: Avenida Valle de Angeles, Calle 5, \nCholuteca, Francisco Morazán',
                imgWidth - margin - 10,
                margin + 30,
                { align: 'right' }
            );
            ...
            doc.setFont(undefined, 'normal');
            doc.text(
                'Municipio: ' + this.cliente?.ciu_Descripcion,
                16 * 2,
                110
            );
            ...
            doc.setFont(undefined, 'normal');
            doc.text(
                'Subtotal: LPS ' + this.compra.com_Precio + '.00',
                140 * 2,
                110
            );
            doc.setFontSize(10);
            doc.setFont(undefined, 'bold');
            doc.text(
                'Total: LPS ' + this.compra.com_Precio + '.00',
                140 * 2,
                120
            );
            ...
            this.messageService.add({
                severity: 'error',
                summary: 'Advertencia',
                detail: `Parece que su factura no tiene detalles`,
                life: 3000,
            });
        }
    ...
    async saveFactura() {
        ...
        if (this.compra.com_Id && this.compra.com_Estado) {
            ...
            await this.CompraService.Update(this.compra).then((result) => {
                if (result.code == 200) {
                    this.cambios = false;
            ...
            this.vehiculosInsert.forEach(async (element) => {
                ...
                await this.VehiculoService.Insert(element).then((result) => {
                    if (result.code == 200) {
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Éxito',
                            detail: 'insercion Correctamente',
                            life: 3000,
                        });
            ...
            this.vehiculosDelete.forEach(async (element) => {
                await this.VehiculoService.Delete(element.veh_Placa).then(
                    (result) => {
                        if (result.code == 200) {
                            this.messageService.add({
                                severity: 'success',
                                summary: 'Éxito',
                                detail: 'deletion Correctamente',
                                life: 3000,
                            });
            ...
            this.vehiculosUpdate.forEach(async (element) => {
                ...
                await this.VehiculoService.Update(element).then((result) => {
                    if (result.code == 200) {
                        if (contador == 0) {
                            this.messageService.add({
                                severity: 'success',
                                summary: 'Éxito',
                                detail: 'update Correctamente',
                                life: 3000,
                            });
                        }
            ...
            await this.CompraService.Insert(this.compra).then((result) => {
                if (result.code == 200) {
                    this.cambios = false;
    ...
    confirmDelete() {
        ...
        this.vehiculos = this.vehiculos.filter(
            (val) => val.veh_Id !== this.vehiculo.veh_Id
        );

        this.vehiculosUpdate = this.vehiculosUpdate.filter(
            (val) => val.veh_Id !== this.vehiculo.veh_Id
        );
